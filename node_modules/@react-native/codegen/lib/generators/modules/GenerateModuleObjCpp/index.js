...
const {
  serializeStruct
} = require('./header/serializeStruct');
const {
  EventEmitterHeaderTemplate
} = require('./serializeEventEmitter');
const {
  serializeMethod
} = require('./serializeMethod');
const {
  serializeModuleSource
} = require('./source/serializeModule');
const {
  StructCollector
} = require('./StructCollector');
...
module.exports = {
  generate(libraryName, schema, packageName, assumeNonnull, headerPrefix) {
    ...
    for (const hasteModuleName of hasteModuleNames) {
      ...
      spec.methods.filter(property => property.name !== 'getConstants').forEach(serializeProperty);
      spec.methods.filter(property => property.name === 'getConstants').forEach(serializeProperty);
      ...
      for (const struct of generatedStructs) {
        const {
          methods,
          declaration
        } = serializeStruct(hasteModuleName, struct);
      ...
      moduleImplementations.push(serializeModuleSource(hasteModuleName, generatedStructs, hasteModuleName, spec.eventEmitters, methodSerializations.filter(({
        selector
