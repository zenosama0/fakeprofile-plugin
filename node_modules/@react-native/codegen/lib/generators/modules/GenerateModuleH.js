...
function translatePrimitiveJSTypeToCpp(moduleName, parentObjectAliasName, nullableTypeAnnotation, optional, createErrorMessage, resolveAlias, enumMap) {
  const [typeAnnotation, nullable] = unwrapNullable(nullableTypeAnnotation);
  const isRecursiveType = isDirectRecursiveMember(parentObjectAliasName, nullableTypeAnnotation);
...
function createStructsString(hasteModuleName, aliasMap, resolveAlias, enumMap) {
  const getCppType = (parentObjectAlias, v) => translatePrimitiveJSTypeToCpp(hasteModuleName, parentObjectAlias, v.typeAnnotation, false, typeName => `Unsupported type for param "${v.name}". Found: ${typeName}`, resolveAlias, enumMap);
  return Object.keys(aliasMap).map(alias => {
    ...
    const templateParameter = value.properties.filter(v => !isDirectRecursiveMember(alias, v.typeAnnotation) && !isArrayRecursiveMember(alias, v.typeAnnotation));
    const templateParameterWithTypename = templateParameter.map((v, i) => `typename P${i}`).join(', ');
    const templateParameterWithoutTypename = templateParameter.map((v, i) => `P${i}`).join(', ');
    let i = -1;
    const templateMemberTypes = value.properties.map(v => {
      if (isDirectRecursiveMember(alias, v.typeAnnotation)) {
        ...
        return (nullable ? `std::optional<std::vector<${structName}<${templateParameterWithoutTypename}>>>` : `std::vector<${structName}<${templateParameterWithoutTypename}>>`) + ` ${v.name}`;
      } else {
        i++;
        return `P${i} ${v.name}`;
      }
    });
    const debugParameterConversion = value.properties.map(v => `  static ${getCppType(alias, v)} ${v.name}ToJs(jsi::Runtime &rt, decltype(types.${v.name}) value) {
...
const EnumTemplate = ({
  ...
  const [fromValue, fromValueConversion, toValue] = nativeEnumMemberType === 'std::string' ? ['const jsi::String &rawValue', 'std::string value = rawValue.utf8(rt);', 'jsi::String'] : ['const jsi::Value &rawValue', 'double value = (double)rawValue.asNumber();', 'jsi::Value'];
  return `
...
function generateEnum(hasteModuleName, origEnumName, members, memberType) {
  ...
  const nativeEnumMemberType = memberType === 'StringTypeAnnotation' ? 'std::string' : 'int32_t';
  const fromCases = members.map(member => `if (value == ${getMemberValueAppearance(member.value)}) {
      return ${enumName}::${toSafeCppString(member.name)};
    }`).join(' else ') + ` else {
      throw jsi::JSError(rt, "No appropriate enum member found for value in ${enumName}");
    }`;
  const toCases = members.map(member => `if (value == ${enumName}::${toSafeCppString(member.name)}) {
...
function createEnums(hasteModuleName, enumMap, resolveAlias) {
  return Object.entries(enumMap).map(([enumName, enumNode]) => {
    return generateEnum(hasteModuleName, enumName, enumNode.members, enumNode.memberType);
  }).filter(Boolean).join('\n');
}
function translateFunctionToCpp(hasteModuleName, prop, resolveAlias, enumMap, args, returnTypeAnnotation) {
  ...
  const paramTypes = propTypeAnnotation.params.map(param => {
    const translatedParam = translatePrimitiveJSTypeToCpp(hasteModuleName, null, param.typeAnnotation, param.optional, typeName => `Unsupported type for param "${param.name}" in ${prop.name}. Found: ${typeName}`, resolveAlias, enumMap);
    return `${translatedParam} ${param.name}`;
  });
  paramTypes.unshift('jsi::Runtime &rt');
  const returnType = translatePrimitiveJSTypeToCpp(hasteModuleName, null, propTypeAnnotation.returnTypeAnnotation, false, typeName => `Unsupported return type for ${prop.name}. Found: ${typeName}`, resolveAlias, enumMap);
...
function translateEventEmitterToCpp(moduleName, eventEmitter, resolveAlias, enumMap) {
  ...
  return {
    ...
    emitFunction: `
  ${isVoidTypeAnnotation ? '' : `template <typename ${templateName}> `}void emit${toPascalCase(eventEmitter.name)}(${isVoidTypeAnnotation ? '' : `${isArray ? `std::vector<${templateName}>` : templateName} value`}) {${isVoidTypeAnnotation ? '' : `
    ...
    static_cast<AsyncEventEmitter<${isVoidTypeAnnotation ? '' : 'jsi::Value'}>&>(*eventEmitterMap_["${eventEmitter.name}"]).emit(${isVoidTypeAnnotation ? '' : `[jsInvoker = jsInvoker_, eventValue = value](jsi::Runtime& rt) -> jsi::Value {
...
module.exports = {
  generate(libraryName, schema, packageName, assumeNonnull = false, headerPrefix) {
    ...
    const modules = Object.keys(nativeModules).flatMap(hasteModuleName => {
      ...
      const structs = createStructsString(hasteModuleName, aliasMap, resolveAlias, enumMap);
      const enums = createEnums(hasteModuleName, enumMap, resolveAlias);
      return [ModuleSpecClassDeclarationTemplate({
        hasteModuleName,
        moduleName,
        structs,
        enums,
        moduleEventEmitters: spec.eventEmitters.map(eventEmitter => translateEventEmitterToCpp(moduleName, eventEmitter, resolveAlias, enumMap)),
        moduleFunctions: spec.methods.map(property => {
          const [propertyTypeAnnotation] = unwrapNullable(property.typeAnnotation);
          return translateFunctionToCpp(hasteModuleName, property, resolveAlias, enumMap, propertyTypeAnnotation.params.map((p, i) => serializeArg(moduleName, p, i, resolveAlias, enumMap)), propertyTypeAnnotation.returnTypeAnnotation);
        }),
        methods: methods.map(({
          name: propertyName,
          typeAnnotation: nullableTypeAnnotation
