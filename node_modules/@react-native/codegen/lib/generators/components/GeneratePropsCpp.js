...
`.trim();
function generatePropsDiffString(className, componentName, component, debugProps = '', includeGetDebugPropsImplementation = false, generateOptionalProperties = false) {
  const diffProps = component.props.map(prop => {
  ...
  if (${prop.name} != oldProps->${prop.name}) {
    result["${prop.name}"] = toDynamic(${prop.name});
          ...
          case 'ImageRequestPrimitive':
            throw new Error('ImageRequestPrimitive should not be used in Props');
          default:
            typeAnnotation.name;
...
module.exports = {
  generate(libraryName, schema, packageName, assumeNonnull = false, headerPrefix, includeGetDebugPropsImplementation = false) {
    ...
    if (includeGetDebugPropsImplementation) {
      ...
      allImports.add('#include <react/renderer/debug/debugStringConvertibleUtils.h>');
    }
    const componentProps = Object.keys(schema.modules).map(moduleName => {
      ...
      return Object.keys(components).map(componentName => {
        ...
        const debugProps = includeGetDebugPropsImplementation ? generateDebugPropsString(componentName, component) : '';
        const diffPropsString = generatePropsDiffString(newName, componentName, component, debugProps, includeGetDebugPropsImplementation, component.generateOptionalProperties);
