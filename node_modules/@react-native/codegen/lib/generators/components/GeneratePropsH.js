...
function generateArrayEnumString(componentName, name, options) {
  ...
  const toCases = options.map(option => `if (wrapped.value & ${enumName}::${toSafeCppString(option)}) {
...
function generateIntEnum(componentName, prop) {
  ...
  if (typeAnnotation.type === 'Int32EnumTypeAnnotation') {
    ...
    const toDynamicCases = values.map(value => `case ${enumName}::${toIntEnumValueName(prop.name, value)}: return ${value};`).join('\n' + '    ');
...
function generatePropsString(componentName, props, nameParts, generateOptionalProperties = false) {
  return props.map(prop => {
    const nativeType = getNativeTypeFromAnnotation(componentName, prop, nameParts);
    const defaultInitializer = getDefaultInitializerString(componentName, prop);
    if (prop.optional && prop.typeAnnotation.default == null && generateOptionalProperties) {
...
function generateStructsForComponent(componentName, component) {
  const structs = generateStructs(componentName, component.props, [], component.generateOptionalObjectProperties);
...
function generateStructs(componentName, properties, nameParts, generateOptionalObjectProperties = false) {
  const structs = new Map();
  properties.forEach(prop => {
    const typeAnnotation = prop.typeAnnotation;
    if (typeAnnotation.type === 'ObjectTypeAnnotation') {
      const elementProperties = typeAnnotation.properties;
      const nestedStructs = generateStructs(componentName, elementProperties, nameParts.concat([prop.name]), generateOptionalObjectProperties);
      nestedStructs.forEach(function (value, key) {
        structs.set(key, value);
      });
      generateStruct(structs, componentName, nameParts.concat([prop.name]), typeAnnotation.properties, generateOptionalObjectProperties);
    }
    if (prop.typeAnnotation.type === 'ArrayTypeAnnotation' && prop.typeAnnotation.elementType.type === 'ObjectTypeAnnotation') {
      ...
      generateStruct(structs, componentName, nameParts.concat([prop.name]), elementProperties, generateOptionalObjectProperties);
...
function generateStruct(structs, componentName, nameParts, properties, generateOptionalObjectProperties = false) {
  const structNameParts = nameParts;
  const structName = generateStructName(componentName, structNameParts);
  const fields = generatePropsString(componentName, properties, structNameParts, generateOptionalObjectProperties);
  properties.forEach(property => {
    ...
    switch (property.typeAnnotation.type) {
      ...
      case 'ObjectTypeAnnotation':
        ...
        generateStruct(structs, componentName, nameParts.concat([name]), props, generateOptionalObjectProperties);
        return;
  ...
  const toDynamicCases = properties.map(property => {
    const name = property.name;
    switch (property.typeAnnotation.type) {
      case 'BooleanTypeAnnotation':
      case 'StringTypeAnnotation':
      ...
      case 'FloatTypeAnnotation':
      case 'MixedTypeAnnotation':
        if (property.optional && property.typeAnnotation.default == null && generateOptionalObjectProperties) {
  ...
  generate(libraryName, schema, packageName, assumeNonnull = false, headerPrefix, includeGetDebugPropsImplementation = false) {
    ...
    const componentClasses = Object.keys(schema.modules).map(moduleName => {
      ...
      return Object.keys(components).map(componentName => {
        ...
        const propsString = generatePropsString(componentName, component.props, [], component.generateOptionalProperties);
