...
const ComponentTemplate = ({
  ...
  const nativeComponentName = paperComponentName !== null && paperComponentName !== void 0 ? paperComponentName : componentName;
  return `
...
function getValidAttributesForEvents(events, imports) {
  ...
  return t.callExpression(t.identifier('ConditionallyIgnoredEventHandlers'), [validAttributes]);
}
function generateBubblingEventInfo(event, nameOveride) {
  return t.objectProperty(t.identifier(normalizeInputEventName(nameOveride || event.name)), t.objectExpression([t.objectProperty(t.identifier('phasedRegistrationNames'), t.objectExpression([t.objectProperty(t.identifier('captured'), t.stringLiteral(`${event.name}Capture`)), t.objectProperty(t.identifier('bubbled'), t.stringLiteral(event.name))]))]));
}
...
function buildViewConfig(schema, componentName, component, imports) {
  ...
  }), ...(componentEvents.length > 0 ? [t.spreadElement(getValidAttributesForEvents(componentEvents, imports))] : [])]);
  ...
  const properties = [t.objectProperty(t.identifier('uiViewClassName'), t.stringLiteral(componentName))];
  if (bubblingEventNames.length > 0) {
    properties.push(t.objectProperty(t.identifier('bubblingEventTypes'), t.objectExpression(bubblingEventNames)));
  }
  if (directEventNames.length > 0) {
    properties.push(t.objectProperty(t.identifier('directEventTypes'), t.objectExpression(directEventNames)));
  }
      ...
      const moduleResults = Object.keys(schema.modules).map(moduleName => {
        ...
        return Object.keys(components).map(componentName => {
          ...
          const replacedSourceRoot = core.template.program(replacedTemplate)({
            VIEW_CONFIG: buildViewConfig(schema, paperComponentName, component, imports)
          });
          const commandsExport = buildCommands(schema, paperComponentName, component, imports);
          if (commandsExport) {
            replacedSourceRoot.body.push(commandsExport);
          }
          const replacedSource = core.transformFromAstSync(replacedSourceRoot, undefined, {
            babelrc: false,
