...
module.exports = {
  generate(schemas, supportedApplePlatforms) {
    ...
    const lookupMap = Object.keys(schemas).map(libraryName => {
      const schema = schemas[libraryName];
      const librarySupportedApplePlatforms = supportedApplePlatforms === null || supportedApplePlatforms === void 0 ? void 0 : supportedApplePlatforms[libraryName];
      const generatedLookup = Object.keys(schema.modules).map(moduleName => {
        ...
        const componentTemplates = Object.keys(components).filter(componentName => {
      ...
      return generateSupportedApplePlatformsMacro(generatedLookup, librarySupportedApplePlatforms);
    }).join('\n');
