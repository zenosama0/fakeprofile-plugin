  ...
  switch (typeAnnotation.type) {
    ...
    case 'ArrayTypeAnnotation':
      ...
      if (eventTypeAnnotation.type !== 'ArrayTypeAnnotation') {
        throw new Error("Inconsistent Codegen state: type was ArrayTypeAnnotation at the beginning of the body and now it isn't");
      }
      return getCppArrayTypeForAnnotation(eventTypeAnnotation.elementType, [...nameParts, eventProperty.name]);
    default:
      typeAnnotation.type;
...
function handleGenerateStructForArray(structs, name, componentName, elementType, nameParts) {
  if (elementType.type === 'ObjectTypeAnnotation') {
    generateStruct(structs, componentName, nameParts.concat([name]), nullthrows(elementType.properties));
  } else if (elementType.type === 'UnionTypeAnnotation') {
    const validUnionType = parseValidUnionType(elementType);
    if (validUnionType !== 'string') {
      throw new Error('Invalid since it is a union of non strings');
    }
    generateEnum(structs, elementType.types.map(literal => literal.value), nameParts.concat([name]));
  } else if (elementType.type === 'ArrayTypeAnnotation') {
    handleGenerateStructForArray(structs, name, componentName, elementType.elementType, nameParts);
  }
}
  ...
  properties.forEach(property => {
    ...
    switch (typeAnnotation.type) {
      ...
      case 'MixedTypeAnnotation':
        return;
      case 'ArrayTypeAnnotation':
        handleGenerateStructForArray(structs, name, componentName, typeAnnotation.elementType, nameParts);
        return;
      case 'ObjectTypeAnnotation':
      ...
      case 'UnionTypeAnnotation':
        ...
        generateEnum(structs, typeAnnotation.types.map(literal => literal.value), nameParts.concat([name]));
        return;
      default:
        typeAnnotation.type;
        throw new Error(`Received invalid event property type ${typeAnnotation.type}`);
...
module.exports = {
  generate(libraryName, schema, packageName, assumeNonnull = false, headerPrefix, includeGetDebugPropsImplementation = false) {
    ...
    const componentEmitters = Object.keys(moduleComponents).map(componentName => {
      ...
      component.events.forEach(event => {
        if (event.typeAnnotation.argument) {
          const argIncludes = getImports(event.typeAnnotation.argument.properties);
