...
function throwIfEventEmitterTypeIsUnsupported(nativeModuleName, propertyName, propertyValueType, parser, nullable, untyped) {
  if (nullable || untyped) {
    throw new UnsupportedModuleEventEmitterPropertyParserError(nativeModuleName, propertyName, propertyValueType, parser.language(), nullable, untyped);
  }
}
function throwIfEventEmitterEventTypeIsUnsupported(nativeModuleName, propertyName, propertyValueType, parser, nullable) {
  if (nullable) {
...
function throwIfMoreThanOneModuleInterfaceParserError(nativeModuleName, moduleSpecs, parserType) {
  if (moduleSpecs.length > 1) {
    throw new MoreThanOneModuleInterfaceParserError(nativeModuleName, moduleSpecs, moduleSpecs.map(node => node.id.name), parserType);
  }
}
...
function throwIfIncorrectModuleRegistryCallArgument(nativeModuleName, callExpressionArg, methodName) {
  ...
  if (typeof value !== 'string' || !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(value)) {
    throw new IncorrectModuleRegistryCallArgumentValueParserError(nativeModuleName, callExpressionArg, methodName, String(value));
  }
}
...
function throwIfEventHasNoName(typeAnnotation, parser) {
  const name = parser.language() === 'Flow' ? typeAnnotation.id : typeAnnotation.typeName;
  if (!name) {
...
function throwIfBubblingTypeIsNull(bubblingType, eventName) {
  if (!bubblingType) {
    throw new Error(`Unable to determine event bubbling type for "${eventName}"`);
  }
  return bubblingType;
}
