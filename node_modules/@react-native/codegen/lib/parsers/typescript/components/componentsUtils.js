...
function getUnionOfLiterals(name, forArray, elementTypes, defaultValue, types) {
  ...
  elementTypes.reduce((lastType, currType) => {
    const lastFlattenedType = lastType && lastType.type === 'TSLiteralType' ? lastType.literal.type : lastType.type;
    const currFlattenedType = currType.type === 'TSLiteralType' ? currType.literal.type : currType.type;
...
function detectArrayType(name, typeAnnotation, defaultValue, types, parser, buildSchema) {
  if (typeAnnotation.type === 'TSTypeOperator' && typeAnnotation.operator === 'readonly' && typeAnnotation.typeAnnotation.type === 'TSArrayType') {
    return {
      ...
      elementType: getTypeAnnotationForArray(name, typeAnnotation.typeAnnotation.elementType, defaultValue, types, parser, buildSchema)
    };
  }
  if (typeAnnotation.type === 'TSArrayType') {
    return {
      type: 'ArrayTypeAnnotation',
      elementType: getTypeAnnotationForArray(name, typeAnnotation.elementType, defaultValue, types, parser, buildSchema)
    };
  }
  if (typeAnnotation.type === 'TSTypeReference' && (parser.getTypeAnnotationName(typeAnnotation) === 'ReadonlyArray' || parser.getTypeAnnotationName(typeAnnotation) === 'Array')) {
    return {
      type: 'ArrayTypeAnnotation',
      elementType: getTypeAnnotationForArray(name, typeAnnotation.typeParameters.params[0], defaultValue, types, parser, buildSchema)
    };
  }
  return null;
}
function buildObjectType(rawProperties, types, parser, buildSchema) {
  ...
  const properties = flattenedProperties.map(prop => buildSchema(prop, types, parser)).filter(Boolean);
  return {
...
function getCommonTypeAnnotation(name, forArray, type, typeAnnotation, defaultValue, types, parser, buildSchema) {
  switch (type) {
    case 'TSTypeLiteral':
      return buildObjectType(typeAnnotation.members, types, parser, buildSchema);
    case 'TSInterfaceDeclaration':
      return buildObjectType([typeAnnotation], types, parser, buildSchema);
    case 'TSIntersectionType':
      return buildObjectType(flattenIntersectionType(typeAnnotation, parser, types), types, parser, buildSchema);
    case 'ImageSource':
      return {
...
function getTypeAnnotationForArray(name, typeAnnotation, defaultValue, types, parser, buildSchema) {
  ...
  const arrayType = detectArrayType(name, extractedTypeAnnotation, defaultValue, types, parser, buildSchema);
  if (arrayType) {
  ...
  const type = extractedTypeAnnotation.elementType === 'TSTypeReference' ? parser.getTypeAnnotationName(extractedTypeAnnotation.elementType) : ((_extractedTypeAnnotat = extractedTypeAnnotation.elementType) === null || _extractedTypeAnnotat === void 0 ? void 0 : _extractedTypeAnnotat.type) || parser.getTypeAnnotationName(extractedTypeAnnotation) || extractedTypeAnnotation.type;
  const common = getCommonTypeAnnotation(name, true, type, extractedTypeAnnotation, defaultValue, types, parser, buildSchema);
  if (common) {
    return common;
  }
  switch (type) {
...
function getTypeAnnotation(name, annotation, defaultValue, withNullDefault, types, parser, buildSchema) {
  const topLevelType = parseTopLevelType(annotation, parser, types);
  const typeAnnotation = topLevelType.type;
  const arrayType = detectArrayType(name, typeAnnotation, defaultValue, types, parser, buildSchema);
  if (arrayType) {
    return arrayType;
  }
  const type = typeAnnotation.type === 'TSTypeReference' || typeAnnotation.type === 'TSTypeAliasDeclaration' ? parser.getTypeAnnotationName(typeAnnotation) : typeAnnotation.type;
  const common = getCommonTypeAnnotation(name, false, type, typeAnnotation, defaultValue, types, parser, buildSchema);
  if (common) {
...
function flattenProperties(typeDefinition, types, parser) {
  return typeDefinition.map(property => {
    if (property.type === 'TSPropertySignature') {
      ...
      return flattenProperties(parser.getProperties(property.typeName.name, types), types, parser);
    } else if (property.type === 'TSExpressionWithTypeArguments' || property.type === 'TSInterfaceHeritage') {
      return flattenProperties(parser.getProperties(property.expression.name, types), types, parser);
      ...
      return flattenProperties(parser.getProperties(property.id.name, types), types, parser);
