...
class TypeScriptParser {
  constructor() {
    ...
    _defineProperty(this, "undefinedLiteralTypeAnnotation", 'TSUndefinedKeyword');
  }
  isProperty(property) {
  ...
  getTypeAnnotationName(typeAnnotation) {
    ...
    if ((typeAnnotation === null || typeAnnotation === void 0 || (_typeAnnotation$typeN = typeAnnotation.typeName) === null || _typeAnnotation$typeN === void 0 ? void 0 : _typeAnnotation$typeN.type) === 'TSQualifiedName') {
      return typeAnnotation.typeName.right.name;
    }
    return typeAnnotation === null || typeAnnotation === void 0 || (_typeAnnotation$typeN2 = typeAnnotation.typeName) === null || _typeAnnotation$typeN2 === void 0 ? void 0 : _typeAnnotation$typeN2.name;
  }
  ...
  parseString(contents, filename) {
    return buildSchema(contents, filename, wrapComponentSchema, buildComponentSchema, buildModuleSchema, Visitor, this, typeScriptTranslateTypeAnnotation);
  }
  ...
  parseEnumMembersType(typeAnnotation) {
    ...
    const enumInitializer = (_typeAnnotation$membe = typeAnnotation.members[0]) === null || _typeAnnotation$membe === void 0 ? void 0 : _typeAnnotation$membe.initializer;
    const enumInitializerType = enumInitializer === null || enumInitializer === void 0 ? void 0 : enumInitializer.type;
  ...
  validateEnumMembersSupported(typeAnnotation, enumMembersType) {
    ...
    const enumInitializerType = enumMembersType === 'StringTypeAnnotation' ? 'StringLiteral' : enumMembersType === 'NumberTypeAnnotation' ? 'NumericLiteral' : null;
    typeAnnotation.members.forEach(member => {
      var _member$initializer, _member$initializer2, _member$initializer3, _member$initializer4;
      const isNegative = ((_member$initializer = member.initializer) === null || _member$initializer === void 0 ? void 0 : _member$initializer.type) === 'UnaryExpression' && ((_member$initializer2 = member.initializer) === null || _member$initializer2 === void 0 ? void 0 : _member$initializer2.operator) === '-';
      const initializerType = isNegative ? (_member$initializer3 = member.initializer) === null || _member$initializer3 === void 0 || (_member$initializer3 = _member$initializer3.argument) === null || _member$initializer3 === void 0 ? void 0 : _member$initializer3.type : (_member$initializer4 = member.initializer) === null || _member$initializer4 === void 0 ? void 0 : _member$initializer4.type;
      if ((initializerType !== null && initializerType !== void 0 ? initializerType : 'StringLiteral') !== enumInitializerType) {
  ...
  parseEnumMembers(typeAnnotation) {
    return typeAnnotation.members.map(member => {
      var _member$initializer5, _member$initializer6, _member$initializer7, _member$initializer8, _member$initializer9, _member$initializer0;
      const value = ((_member$initializer5 = member.initializer) === null || _member$initializer5 === void 0 ? void 0 : _member$initializer5.operator) === '-' ? {
        type: 'NumberLiteralTypeAnnotation',
        value: -1 * ((_member$initializer6 = member.initializer) === null || _member$initializer6 === void 0 || (_member$initializer6 = _member$initializer6.argument) === null || _member$initializer6 === void 0 ? void 0 : _member$initializer6.value)
      } : typeof ((_member$initializer7 = member.initializer) === null || _member$initializer7 === void 0 ? void 0 : _member$initializer7.value) === 'number' ? {
        type: 'NumberLiteralTypeAnnotation',
        value: (_member$initializer8 = member.initializer) === null || _member$initializer8 === void 0 ? void 0 : _member$initializer8.value
      } : typeof ((_member$initializer9 = member.initializer) === null || _member$initializer9 === void 0 ? void 0 : _member$initializer9.value) === 'string' ? {
        type: 'StringLiteralTypeAnnotation',
        value: (_member$initializer0 = member.initializer) === null || _member$initializer0 === void 0 ? void 0 : _member$initializer0.value
      } : {
        type: 'StringLiteralTypeAnnotation',
  ...
  computePartialProperties(properties, hasteModuleName, types, aliasMap, enumMap, tryParse, cxxOnly) {
    return properties.map(prop => {
      return {
        name: prop.key.name,
        optional: true,
        typeAnnotation: typeScriptTranslateTypeAnnotation(hasteModuleName, prop.typeAnnotation.typeAnnotation, types, aliasMap, enumMap, tryParse, cxxOnly, this)
      };
    });
  }
  ...
  getResolvedTypeAnnotation(typeAnnotation, types, parser) {
    ...
    let node = typeAnnotation.type === 'TSTypeAnnotation' ? typeAnnotation.typeAnnotation : typeAnnotation;
    let nullable = false;
  ...
  isProp(name, typeAnnotation) {
    ...
    const isStyle = name === 'style' && typeAnnotation.type === 'GenericTypeAnnotation' && this.getTypeAnnotationName(typeAnnotation) === 'ViewStyleProp';
    return !isStyle;
  }
  getProps(typeDefinition, types) {
    ...
    for (const prop of flattenProperties(remaining, types, this)) {
      const topLevelType = parseTopLevelType(prop.typeAnnotation.typeAnnotation, this, types);
      if (prop.type === 'TSPropertySignature' && !this.isEvent(topLevelType.type) && this.isProp(prop.key.name, prop)) {
        componentPropAsts.push(prop);
      }
    }
    return {
      props: componentPropAsts.map(property => buildPropSchema(property, types, this)).filter(Boolean),
      extendsProps
    };
  }
  getProperties(typeName, types) {
    ...
    const aliasKind = alias.type === 'TSInterfaceDeclaration' ? 'interface' : 'type';
    try {
      if (aliasKind === 'interface') {
        var _alias$extends;
        return [...((_alias$extends = alias.extends) !== null && _alias$extends !== void 0 ? _alias$extends : []), ...alias.body.body];
      }
  ...
  extractTypeFromTypeAnnotation(typeAnnotation) {
    return typeAnnotation.type === 'TSTypeReference' ? typeAnnotation.typeName.name : typeAnnotation.type;
  }
  ...
  getPaperTopLevelNameDeprecated(typeAnnotation) {
    return typeAnnotation.typeParameters.params.length > 1 ? typeAnnotation.typeParameters.params[1].literal.value : null;
  }
}
module.exports = {
  TypeScriptParser
