  ...
  switch (type.type) {
    ...
    case 'TSTypeReference':
      if (type.typeName.name === 'Readonly') {
        handleUnionAndParen(type.typeParameters.params[0], result, parser, knownTypes);
...
function handleIntersectionAndParen(type, result, parser, knownTypes) {
  switch (type.type) {
    case 'TSParenthesizedType':
      {
        handleIntersectionAndParen(type.typeAnnotation, result, parser, knownTypes);
        break;
      }
    ...
    case 'TSTypeReference':
      if (type.typeName.name === 'Readonly') {
        handleIntersectionAndParen(type.typeParameters.params[0], result, parser, knownTypes);
      } else if (parser.getTypeAnnotationName(type) === 'WithDefault') {
