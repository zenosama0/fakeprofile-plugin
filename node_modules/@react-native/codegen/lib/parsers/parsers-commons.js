...
function getObjectTypeAnnotations(hasteModuleName, types, tryParse, translateTypeAnnotation, parser) {
  const aliasMap = {};
...
function parseObjectProperty(parentObject, property, hasteModuleName, types, aliasMap, enumMap, tryParse, cxxOnly, nullable, translateTypeAnnotation, parser) {
  ...
  if (parentObject) {
    ...
    const propertyType = parser.getResolveTypeAnnotationFN()(languageTypeAnnotation, types, parser);
    if (propertyType.typeResolutionStatus.successful === true && propertyType.typeResolutionStatus.type === 'alias' && (language === 'TypeScript' ? parentObject.typeName && parentObject.typeName.name === ((_languageTypeAnnotati = languageTypeAnnotation.typeName) === null || _languageTypeAnnotati === void 0 ? void 0 : _languageTypeAnnotati.name) : parentObject.id && parentObject.id.name === ((_languageTypeAnnotati2 = languageTypeAnnotation.id) === null || _languageTypeAnnotati2 === void 0 ? void 0 : _languageTypeAnnotati2.name))) {
      if (!optional) {
        throw new UnsupportedObjectDirectRecursivePropertyParserError(name, languageTypeAnnotation, hasteModuleName);
      }
      return {
        name,
        optional,
...
function translateFunctionTypeAnnotation(hasteModuleName, functionTypeAnnotation, types, aliasMap, enumMap, tryParse, cxxOnly, translateTypeAnnotation, parser) {
  const params = [];
  for (const param of parser.getFunctionTypeAnnotationParameters(functionTypeAnnotation)) {
    const parsedParam = tryParse(() => {
      ...
      const [paramTypeAnnotation, isParamTypeAnnotationNullable] = unwrapNullable(translateTypeAnnotation(hasteModuleName, parser.getParameterTypeAnnotation(param), types, aliasMap, enumMap, tryParse, cxxOnly, parser));
  ...
  const [returnTypeAnnotation, isReturnTypeAnnotationNullable] = unwrapNullable(translateTypeAnnotation(hasteModuleName, parser.getFunctionTypeAnnotationReturnType(functionTypeAnnotation), types, aliasMap, enumMap, tryParse, cxxOnly, parser));
...
function buildPropertySchema(hasteModuleName, property, types, aliasMap, enumMap, tryParse, cxxOnly, translateTypeAnnotation, parser) {
  let nullable = false;
  let {
    key,
    value
  ...
  } = resolveTypeAnnotationFN(value, types, parser));
...
function buildEventEmitterSchema(hasteModuleName, property, types, aliasMap, enumMap, tryParse, cxxOnly, translateTypeAnnotation, parser) {
  const {
    key
  } = property;
  const value = parser.language() === 'TypeScript' ? property.typeAnnotation.typeAnnotation : property.value;
  const eventemitterName = key.name;
  const resolveTypeAnnotationFN = parser.getResolveTypeAnnotationFN();
  const [typeAnnotation, typeAnnotationNullable] = unwrapNullable(value);
  const typeAnnotationUntyped = value.typeParameters.params.length === 1 && parser.language() === 'TypeScript' ? value.typeParameters.params[0].type === 'TSTypeLiteral' && value.typeParameters.params[0].members.length === 0 : value.typeParameters.params[0].type === 'ObjectTypeAnnotation' && value.typeParameters.params[0].properties.length === 0;
  throwIfEventEmitterTypeIsUnsupported(hasteModuleName, key.name, typeAnnotation.type, parser, typeAnnotationNullable, typeAnnotationUntyped);
...
function buildSchemaFromConfigType(configType, filename, ast, wrapComponentSchema, buildComponentSchema, buildModuleSchema, parser, translateTypeAnnotation) {
  switch (configType) {
    case 'component':
      {
    ...
    case 'module':
      {
        ...
        const schema = tryParse(() => buildModuleSchema(nativeModuleName, ast, tryParse, parser, translateTypeAnnotation));
...
function buildSchema(contents, filename, wrapComponentSchema, buildComponentSchema, buildModuleSchema, Visitor, parser, translateTypeAnnotation) {
  ...
  return buildSchemaFromConfigType(configType, filename, ast, wrapComponentSchema, buildComponentSchema, buildModuleSchema, parser, translateTypeAnnotation);
}
...
const buildModuleSchema = (hasteModuleName, ast, tryParse, parser, translateTypeAnnotation) => {
  const language = parser.language();
  const types = parser.getTypes(ast);
  const moduleSpecs = Object.values(types).filter(t => parser.isModuleInterface(t));
  ...
  const aliasMap = cxxOnly ? getObjectTypeAnnotations(hasteModuleName, types, tryParse, translateTypeAnnotation, parser) : {};
  const properties = language === 'Flow' ? moduleSpec.body.properties : moduleSpec.body.body;
  const nativeModuleSchema = properties.filter(property => property.type === 'ObjectTypeProperty' || property.type === 'TSPropertySignature' || property.type === 'TSMethodSignature').map(property => {
    var _property$typeAnnotat, _property$value;
    const enumMap = {};
    const isEventEmitter = language === 'TypeScript' ? (property === null || property === void 0 ? void 0 : property.type) === 'TSPropertySignature' && parser.getTypeAnnotationName(property === null || property === void 0 || (_property$typeAnnotat = property.typeAnnotation) === null || _property$typeAnnotat === void 0 ? void 0 : _property$typeAnnotat.typeAnnotation) === 'EventEmitter' : (property === null || property === void 0 || (_property$value = property.value) === null || _property$value === void 0 ? void 0 : _property$value.type) === 'GenericTypeAnnotation' && parser.getTypeAnnotationName(property === null || property === void 0 ? void 0 : property.value) === 'EventEmitter';
    return tryParse(() => ({
      aliasMap,
      enumMap,
      propertyShape: isEventEmitter ? {
        type: 'eventEmitter',
        value: buildEventEmitterSchema(hasteModuleName, property, types, aliasMap, enumMap, tryParse, cxxOnly, translateTypeAnnotation, parser)
      } : {
        type: 'method',
        value: buildPropertySchema(hasteModuleName, property, types, aliasMap, enumMap, tryParse, cxxOnly, translateTypeAnnotation, parser)
      }
    }));
    ...
    spec: {
      eventEmitters: [...moduleSchema.spec.eventEmitters].concat(propertyShape.type === 'eventEmitter' ? [propertyShape.value] : []),
      methods: [...moduleSchema.spec.methods].concat(propertyShape.type === 'method' ? [propertyShape.value] : [])
    },
...
function findNativeComponentType(statement, foundConfigs, parser) {
  ...
  if (declaration.type === 'TSAsExpression' || declaration.type === 'AsExpression' || declaration.type === 'TypeCastExpression') {
    declaration = declaration.expression;
  }
  try {
    if (declaration.callee.name === 'codegenNativeComponent') {
      const typeArgumentParams = parser.getTypeArgumentParamsFromDeclaration(declaration);
      const funcArgumentParams = declaration.arguments;
      const nativeComponentType = parser.getNativeComponentType(typeArgumentParams, funcArgumentParams);
...
function getCommandTypeNameAndOptionsExpression(namedExport, parser) {
  ...
  const typeArgumentParam = parser.getTypeArgumentParamsFromDeclaration(callExpression)[0];
...
function propertyNames(properties) {
  return properties.map(property => property && property.key && property.key.name).filter(Boolean);
}
...
function buildPropSchema(property, types, parser) {
  ...
  return {
    name,
    optional,
    typeAnnotation: getTypeAnnotationFN(name, typeAnnotation, defaultValue, withNullDefault, types, parser, buildPropSchema)
  };
}
function getEventArgument(argumentProps, parser, getPropertyType) {
  return {
    type: 'ObjectTypeAnnotation',
    properties: argumentProps.map(member => buildPropertiesForEvent(member, parser, getPropertyType))
  };
}
function findComponentConfig(ast, parser) {
  ...
  const commandsTypeNames = namedExports.map(statement => getCommandTypeNameAndOptionsExpression(statement, parser)).filter(Boolean);
...
function getCommandProperties(ast, parser) {
  ...
  if (!properties) {
    throw new Error(`Failed to find type definition for "${commandTypeName}", please check that you have a valid codegen file`);
  }
  ...
  if (commandOptions.supportedCommands.length !== commandPropertyNames.length || !commandOptions.supportedCommands.every(supportedCommand => commandPropertyNames.includes(supportedCommand))) {
    throw new Error(`codegenNativeCommands expected the same supportedCommands specified in the ${commandTypeName} interface: ${commandPropertyNames.join(', ')}`);
  }
  return properties;
}
...
function handleGenericTypeAnnotation(typeAnnotation, resolvedTypeAnnotation, parser) {
  ...
  switch (resolvedTypeAnnotation.type) {
    ...
    case 'OpaqueType':
      {
        typeResolutionStatus = getTypeResolutionStatus('alias', typeAnnotation, parser);
        node = parser.nextNodeForTypeAlias(resolvedTypeAnnotation);
        break;
      }
    case parser.enumDeclaration:
      {
        typeResolutionStatus = getTypeResolutionStatus('enum', typeAnnotation, parser);
    ...
    default:
      {
        throw new TypeError(parser.genericTypeAnnotationErrorMessage(resolvedTypeAnnotation));
      }
  }
  return {
    typeAnnotation: node,
...
function handleEventHandler(name, typeAnnotation, parser, types, findEventArgumentsAndType) {
  const eventType = name === 'BubblingEventHandler' ? 'bubble' : 'direct';
  const paperTopLevelNameDeprecated = parser.getPaperTopLevelNameDeprecated(typeAnnotation);
  switch (typeAnnotation.typeParameters.params[0].type) {
    ...
    default:
      return findEventArgumentsAndType(parser, typeAnnotation.typeParameters.params[0], types, eventType, paperTopLevelNameDeprecated);
  }
}
function emitBuildEventSchema(paperTopLevelNameDeprecated, name, optional, nonNullableBubblingType, argument) {
