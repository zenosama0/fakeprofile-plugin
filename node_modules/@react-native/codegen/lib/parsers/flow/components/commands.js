...
function buildCommandSchema(property, types, parser) {
  ...
  const params = value.params.slice(1).map(param => {
    ...
    const type = paramValue.type === 'GenericTypeAnnotation' ? parser.getTypeAnnotationName(paramValue) : paramValue.type;
    let returnType;
    switch (type) {
      case 'RootTag':
        returnType = {
      ...
      case 'ReadonlyArray':
        if (!paramValue.type === 'GenericTypeAnnotation') {
          throw new Error('Array, $ReadOnlyArray and ReadonlyArray are GenericTypeAnnotation for array');
        }
        returnType = {
...
function getCommandArrayElementTypeType(inputType, parser) {
  ...
  const type = inputType === null || inputType === void 0 ? void 0 : inputType.type;
  ...
  switch (type) {
    ...
    case 'GenericTypeAnnotation':
      const name = typeof inputType.id === 'object' ? parser.getTypeAnnotationName(inputType) : null;
      if (typeof name !== 'string') {
        throw new Error('Expected GenericTypeAnnotation AST name to be a string');
      }
      switch (name) {
        case 'Int32':
          return {
...
function getCommands(commandTypeAST, types, parser) {
  return commandTypeAST.filter(property => property.type === 'ObjectTypeProperty').map(property => buildCommandSchema(property, types, parser)).filter(Boolean);
}
module.exports = {
  getCommands
};