...
function getTypeAnnotationForArray(name, typeAnnotation, defaultValue, types, parser, buildSchema) {
  ...
  if (extractedTypeAnnotation.type === 'GenericTypeAnnotation' && parser.getTypeAnnotationName(extractedTypeAnnotation) === 'WithDefault') {
  ...
  if (extractedTypeAnnotation.type === 'GenericTypeAnnotation') {
    ...
    if (objectType.id.name === '$ReadOnly' || objectType.id.name === 'Readonly') {
      return {
        ...
        properties: flattenProperties(objectType.typeParameters.params[0].properties, types, parser).map(prop => buildSchema(prop, types, parser)).filter(Boolean)
      };
    }
    if (objectType.id.name === '$ReadOnlyArray' || objectType.id.name === 'ReadonlyArray') {
      const nestedObjectType = getValueFromTypes(objectType.typeParameters.params[0], types);
      return {
        type: 'ArrayTypeAnnotation',
        elementType: {
          type: 'ObjectTypeAnnotation',
          properties: flattenProperties(nestedObjectType.typeParameters.params[0].properties, types, parser).map(prop => buildSchema(prop, types, parser)).filter(Boolean)
        }
      };
    }
  }
  const type = extractedTypeAnnotation.type === 'GenericTypeAnnotation' ? parser.getTypeAnnotationName(extractedTypeAnnotation) : extractedTypeAnnotation.type;
  switch (type) {
    case 'ImageSource':
      return {
...
function flattenProperties(typeDefinition, types, parser) {
  return typeDefinition.map(property => {
    if (property.type === 'ObjectTypeProperty') {
      ...
      return flattenProperties(parser.getProperties(property.argument.id.name, types), types, parser);
    }
  }).reduce((acc, item) => {
...
function getTypeAnnotation(name, annotation, defaultValue, withNullDefault, types, parser, buildSchema) {
  ...
  if (typeAnnotation.type === 'GenericTypeAnnotation' && (parser.getTypeAnnotationName(typeAnnotation) === '$ReadOnlyArray' || parser.getTypeAnnotationName(typeAnnotation) === 'ReadonlyArray')) {
    return {
      type: 'ArrayTypeAnnotation',
      elementType: getTypeAnnotationForArray(name, typeAnnotation.typeParameters.params[0], defaultValue, types, parser, buildSchema)
    };
  }
  if (typeAnnotation.type === 'GenericTypeAnnotation' && (parser.getTypeAnnotationName(typeAnnotation) === '$ReadOnly' || parser.getTypeAnnotationName(typeAnnotation) === 'Readonly')) {
    return {
      type: 'ObjectTypeAnnotation',
      properties: flattenProperties(typeAnnotation.typeParameters.params[0].properties, types, parser).map(prop => buildSchema(prop, types, parser)).filter(Boolean)
    };
  }
  const type = typeAnnotation.type === 'GenericTypeAnnotation' ? parser.getTypeAnnotationName(typeAnnotation) : typeAnnotation.type;
  switch (type) {
    case 'ImageSource':
      return {
        type: 'ReservedPropTypeAnnotation',
...
function getSchemaInfo(property, types, parser) {
  ...
  let typeAnnotation = value.type === 'NullableTypeAnnotation' ? value.typeAnnotation : value;
  let typeAnnotationName = parser.getTypeAnnotationName(typeAnnotation);
  const optional = value.type === 'NullableTypeAnnotation' || property.optional || value.type === 'GenericTypeAnnotation' && typeAnnotationName === 'WithDefault';
  if (!property.optional && value.type === 'GenericTypeAnnotation' && typeAnnotationName === 'WithDefault') {
    throw new Error(`key ${name} must be optional if used with WithDefault<> annotation`);
  }
  if (value.type === 'NullableTypeAnnotation' && typeAnnotation.type === 'GenericTypeAnnotation' && typeAnnotationName === 'WithDefault') {
  ...
  if (type === 'GenericTypeAnnotation' && (typeAnnotationName === 'DirectEventHandler' || typeAnnotationName === 'BubblingEventHandler')) {
    return null;
  }
  if (name === 'style' && type === 'GenericTypeAnnotation' && typeAnnotationName === 'ViewStyleProp') {
    return null;
  }
  let defaultValue = null;
  let withNullDefault = false;
  if (type === 'GenericTypeAnnotation' && typeAnnotationName === 'WithDefault') {
    ...
    type = typeAnnotation.type === 'GenericTypeAnnotation' ? typeAnnotationName : typeAnnotation.type;
