...
class FlowParser {
  constructor() {
    ...
    _defineProperty(this, "nullLiteralTypeAnnotation", 'NullLiteralTypeAnnotation');
    _defineProperty(this, "undefinedLiteralTypeAnnotation", 'VoidLiteralTypeAnnotation');
  }
  isProperty(property) {
  ...
  getTypeAnnotationName(typeAnnotation) {
    ...
    if ((typeAnnotation === null || typeAnnotation === void 0 || (_typeAnnotation$id = typeAnnotation.id) === null || _typeAnnotation$id === void 0 ? void 0 : _typeAnnotation$id.type) === 'QualifiedTypeIdentifier') {
      return typeAnnotation.id.id.name;
    }
    return typeAnnotation === null || typeAnnotation === void 0 || (_typeAnnotation$id2 = typeAnnotation.id) === null || _typeAnnotation$id2 === void 0 ? void 0 : _typeAnnotation$id2.name;
  }
  ...
  parseString(contents, filename) {
    return buildSchema(contents, filename, wrapComponentSchema, buildComponentSchema, buildModuleSchema, Visitor, this, flowTranslateTypeAnnotation);
  }
  ...
  parseEnumMembersType(typeAnnotation) {
    const enumMembersType = typeAnnotation.type === 'EnumStringBody' ? 'StringTypeAnnotation' : typeAnnotation.type === 'EnumNumberBody' ? 'NumberTypeAnnotation' : null;
    if (!enumMembersType) {
  ...
  parseEnumMembers(typeAnnotation) {
    return typeAnnotation.members.map(member => {
      ...
      const value = typeof ((_member$init = member.init) === null || _member$init === void 0 ? void 0 : _member$init.value) === 'number' ? {
        type: 'NumberLiteralTypeAnnotation',
        value: member.init.value
      } : typeof ((_member$init2 = member.init) === null || _member$init2 === void 0 ? void 0 : _member$init2.value) === 'string' ? {
  ...
  getTypes(ast) {
    return ast.body.reduce((types, node) => {
      if (node.type === 'ExportNamedDeclaration' && node.exportKind === 'type') {
        if (node.declaration != null && (node.declaration.type === 'TypeAlias' || node.declaration.type === 'OpaqueType' || node.declaration.type === 'InterfaceDeclaration')) {
        ...
        types[node.declaration.id.name] = node.declaration;
      } else if (node.type === 'TypeAlias' || node.type === 'OpaqueType' || node.type === 'InterfaceDeclaration' || node.type === 'EnumDeclaration') {
  ...
  isOptionalProperty(property) {
    return property.value.type === 'NullableTypeAnnotation' || property.optional;
  }
  getGetSchemaInfoFN() {
    return getSchemaInfo;
  }
  getTypeAnnotationFromProperty(property) {
    return property.value.type === 'NullableTypeAnnotation' ? property.value.typeAnnotation : property.value;
  }
  ...
  getResolveTypeAnnotationFN() {
    return (typeAnnotation, types, parser) => this.getResolvedTypeAnnotation(typeAnnotation, types, parser);
  }
  ...
  removeKnownExtends(typeDefinition, types) {
    return typeDefinition.filter(prop => prop.type !== 'ObjectTypeSpreadProperty' || this.extendsForProp(prop, types, this) === null);
  }
  getExtendsProps(typeDefinition, types) {
    return typeDefinition.filter(prop => prop.type === 'ObjectTypeSpreadProperty').map(prop => this.extendsForProp(prop, types, this)).filter(Boolean);
  }
  getProps(typeDefinition, types) {
    const nonExtendsProps = this.removeKnownExtends(typeDefinition, types);
    const props = flattenProperties(nonExtendsProps, types, this).map(property => buildPropSchema(property, types, this)).filter(Boolean);
    return {
      props,
  ...
  extractTypeFromTypeAnnotation(typeAnnotation) {
    return typeAnnotation.type === 'GenericTypeAnnotation' ? typeAnnotation.id.name : typeAnnotation.type;
  }
  ...
  getPaperTopLevelNameDeprecated(typeAnnotation) {
    return typeAnnotation.typeParameters.params.length > 1 ? typeAnnotation.typeParameters.params[1].value : null;
  }
}
module.exports = {
  FlowParser
